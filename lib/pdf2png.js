...
exports.convert = function() {
	...
	filesource.getDataPath(filepathOrData, function(resp){
		...
		tmp.file({ postfix: ".png" }, function(err, imageFilepath, fd) {
			...
			exec("gs -dQUIET -dPARANOIDSAFER -dBATCH -dNOPAUSE -dNOPROMPT -sDEVICE=png16m -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -r" + options.quality + " -dFirstPage=1 -dLastPage=1 -sOutputFile=" + imageFilepath + " \"" + resp.data + "\"", function (error, stdout, stderr) {				// Remove temp files
				resp.clean();
